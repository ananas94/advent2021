...
fn make_tree(it:&mut Iter<Part>) -> TreeNode {
    let left_part = it.next().expect("...");
    let left = match left_part {
        Part::Open      => make_tree(it), 
    ...
    TreeNode::Node{node:Box::new(Tree{ left: left,  right: right})}
}
...
fn main() {
    let mut inputs : Vec<Vec<Part>> = Vec::new();
    ...
    for i in 0..il {
        for j in 0..il {
            ...
            let mut ait = a.iter();
            ait.next();
            let atn = make_tree(&mut ait);   //build it every time cause I do sum in-place...
            let mut bit = b.iter();
            bit.next();
            let btn = make_tree(&mut bit);
            let sum = add_tree_nodes(atn, btn);
            let magn = magnitude(&sum);
