...
fn get_line() -> Result<Vec<u32>, Box<dyn Error>> {
    ...
    let line: Vec<u32> = lineStr
        .iter()
        .map(|str| str.parse())
        .take_while(|x| x.is_ok())
        .map(|x| x.unwrap())
        .collect();
    if line.len() != lineStr.len() || line.len() != 5 {
        return Err("BBB")?;
    }
...
fn main() -> Result<(), Box<dyn Error>> {
        ...
        for row in &mut board_data {
            match row.row.iter().position(|&x| x == n) {
                Some(v) => {
                    ...
                    row.row[v] = 100;
                    if row.count == 5 {
                        println!("WIN: row.board_number {}", row.board_number);
                        winner = Some(row.board_number);
                        fin = Some(n);
                        winners.insert(row.board_number);
                    }
                }
                None => {}
            }
            if winners.len() == board_number as usize {
                break 'ext;
            }
        }
    }
    ...
    if winner != None {
        for row in board_data {
            for i in row.row {
                if row.board_number == winner.unwrap() {
                    sum += i % 100;
                }
            }
        }
