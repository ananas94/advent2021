...
fn get_days() -> Option<[u64;9] > {
    ...
    let mut days_counter: [u64;9] = [0;9];
    let mut total = 0;
    days.iter().map(|str|->Result<u64,_> { str.parse() }).take_while(|x| x.is_ok()).map(|x| x.unwrap()).for_each(|x| { if x<9 && x>=0 {days_counter[x as usize]+=1; total+=1;} } );
