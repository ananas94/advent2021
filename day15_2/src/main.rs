...
fn extend(cave: &Vec<Vec<u8>>) -> Vec<Vec<u8>> {
    let mut c: Vec<Vec<u8>> = vec![vec![0; cave.len() * 5]; cave[0].len() * 5];
    let dx = cave.len();
    let dy = cave[0].len();
    ...
    for i in 0..5 as usize {
        for j in 0..5 as usize {
            for ik in 0..dx {
                for jk in 0..dy {
                    let val = cave[ik][jk] + i as u8 + j as u8;
                    let dv = val/10;
                    let mv = val%10;
                    c[dx*i+ik][dy*j+jk] = dv + mv;
                }
            }
        }
    }
...
fn main() ->Result<(), Box<dyn Error>> {
    let itintal_cave = get_matrix().ok_or("bad_inp")?;

    let cave = extend(&itintal_cave);
